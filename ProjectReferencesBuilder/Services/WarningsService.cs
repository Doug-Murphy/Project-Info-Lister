...
namespace ProjectReferencesBuilder.Services
{
    public class WarningsService
    {
        ...
        private bool IsProjectTfmEndOfLife(ProjectInfo project, out DateTime? eolDate)
        {
            if (_targetFrameworksWithEndOfLifeDate.TryGetValue(project.TFM, out eolDate))
            {
                return eolDate.HasValue && eolDate < DateTime.Now;
            }
        ...
        public List<Warning> GetWarnings(IEnumerable<ProjectInfo> projects)
        {
            ...
            foreach (var project in projects)
            {
                ...
                if (IsProjectTfmEndOfLife(project, out DateTime? eolDate))
                {
                    warningsForProject.Add(WarningMessageFactory.GetEndOfLifeWarning(project, eolDate.Value));
                }
